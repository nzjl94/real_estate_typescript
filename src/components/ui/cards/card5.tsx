...
const CARD5 = ({id,star,image,title,content,name,location}:CARD5_TYPE) => {
    ...
    return <div key={id} className='grid grid-flow-row gap-y-[40px] p-[20px] md:p-[30px] lg:p-[40px] xl:p-[50px] rounded-[12px] cp-color-1' >
        <div className="flex flex-row gap-x-[10px] items-start ">
            {
                [...Array(star).keys()].map((value)=>(
                    <IMAGE responsive="set2" path={"/image/home/client/star.svg"} activeBorder={false} 
                        parentClass="w-fit p-[6px] lg:p-[8px] xl:p-[10px] rounded-[100px] cp-color-2" 
                    />
                ))
            }
        </div>
        ...
        <div className="grid grid-flow-row grid-cols-5 gap-x-[14px]">
            <IMAGE responsive="set3" path={image} activeBorder={false} parentClass="row-span-2 col-span-1 w-fit place-self-center" />
