...
const CARD_STYLE_5 = ({id,star,image,title,content,name,location}:COMPONET_TYPE) => {
    ...
    return <Item key={id} className='grid grid-flow-row gap-y-[40px] p-[50px] rounded-[12px] bg-gray-2 border border-gray-1' >
        <div className="flex flex-row gap-x-[10px] items-start ">
            {
                [...Array(star).keys()].map((value)=>(
                    <IMG 
                        ...
                        parentClass="w-fit bg-gray-1 p-[10px] rounded-[100px] border border-gray-1" 
                    />
                ))
            }
        ...
        <div className="grid grid-flow-row grid-cols-5 gap-x-[14px]">
            <IMG width={60} height={60} path={image} activeBorder={false} parentClass="row-span-2 col-span-1 w-fit" />
